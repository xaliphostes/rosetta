...
private:
    ...
    std::vector<std::string> topological_sort(const std::vector<std::string>& names,
        ...
        for (const auto& n : names) visit(n);
        return sorted;
    }
    ...
    void write_constructor(const std::string& cpp_type,
        ...
        if (params.empty()) {
            ...
            bool needs_wrapper = std::any_of(params.begin(), params.end(),
    ...
    void write_field(const std::string& cpp_type,
        ...
        if (needs_conversion(type_str)) {
            ...
            oss << "        },\n";
            oss << "        +[](" << cpp_type << "& self, " << get_js_param_type(type_str) << " val) {\n";
    ...
    void write_method(const std::string& cpp_type,
    ...
    bool needs_conversion(const std::string& type) {
        if (type.find("vector<") != std::string::npos) return true;
        if (type.find("function") != std::string::npos) return true;
        
        // Check if it's a registered class type that needs wrapping
        std::string elem = extract_vector_element_type(type);
        if (!elem.empty() && is_registered_class(elem)) return true;
        
        return false;
    }

    std::string get_js_param_type(const std::string& type) {
        if (is_vector_type(type)) return "val";
        if (type.find("function") != std::string::npos) return "val";
        
        // For registered classes that need conversion
        std::string norm = normalize_type(type);
        if (is_registered_class(norm)) return "val";
