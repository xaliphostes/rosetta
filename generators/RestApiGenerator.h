...
private:
    ...
    void write_create_handler(const std::string& name,
            ...
            for (const auto& ctor : valid_ctors) {
                ...
                if (params_types.empty()) {
                    ...
                    for (size_t i = 0; i < params_types.size(); ++i) {
                        args.push_back(generate_json_extraction(params_types[i], "params[" + std::to_string(i) + "]"));
                    }
    ...
    void write_method_handlers(const std::string& name,
                               const std::string& cpp_type,
                               const rosetta::core::Registry::MetadataHolder* holder) {
        line("json call_method(const std::string& id, const std::string& method, const json& params) {");
        indent();
        ...
        for (const auto& m : holder->get_methods()) {
            ...
            if (!param_types.empty()) {
                line("if (!params.is_array() || params.size() != " + std::to_string(param_types.size()) + ") {");
                indent();
                line("return error_response(\"Expected " + std::to_string(param_types.size()) + " parameters for " + m + "\");");
                dedent();
                line("}");
            }
            ...
            for (size_t i = 0; i < param_types.size(); ++i) {
                args.push_back(generate_json_extraction(param_types[i], "params[" + std::to_string(i) + "]"));
            }
            ...
            if (return_type == "void") {
                ...
                line("return success_response(" + generate_json_conversion(return_type, "result") + ");");
            }
    ...
    void write_server_setup() {
        ...
        indent();
        line("res.set_content(json{{\"status\", \"ok\"}, {\"module\", \"" + config_.module_name + "\"}}.dump(), \"application/json\");");
        dedent();
        line("});");
        line();
        ...
        line("server.Get(\"/api/classes/:name\", [](const httplib::Request& req, httplib::Response& res) {");
        indent();
        ...
        for (const auto& name : registry.list_classes()) {
            ...
            line(cond + " (name == \"" + name + "\") {");
            indent();
            line("res.set_content(success_response(" + name + "Handler::get_info()).dump(), \"application/json\");");
            dedent();
        }
        if (!first) {
            line("} else {");
            indent();
            line("res.status = 404;");
            line("res.set_content(error_response(\"Class not found: \" + name, 404).dump(), \"application/json\");");
            dedent();
            line("}");
        }
        dedent();
        ...
        indent();
        line("res.set_content(success_response(ObjectStore::instance().list_objects()).dump(), \"application/json\");");
        dedent();
        line("});");
        line();
        ...
        line("server.Post(\"/api/objects/:class\", [](const httplib::Request& req, httplib::Response& res) {");
        indent();
        ...
        for (const auto& name : registry.list_classes()) {
            ...
            line(cond + " (cls == \"" + name + "\") {");
            indent();
            line("res.set_content(" + name + "Handler::create(params).dump(), \"application/json\");");
            dedent();
        }
        if (!first) {
            line("} else {");
            indent();
            line("res.status = 404;");
            line("res.set_content(error_response(\"Unknown class: \" + cls, 404).dump(), \"application/json\");");
            dedent();
            line("}");
        }
        dedent();
        ...
        line("server.Post(\"/api/objects/:id/:method\", [](const httplib::Request& req, httplib::Response& res) {");
        indent();
        ...
        indent();
        line("res.status = 404;");
        line("res.set_content(error_response(\"Object not found: \" + id, 404).dump(), \"application/json\");");
        line("return;");
        dedent();
        line("}");
        line();
        ...
        for (const auto& name : registry.list_classes()) {
            ...
            line(cond + " (cls == \"" + name + "\") {");
            indent();
            line("res.set_content(" + name + "Handler::call_method(id, method, params).dump(), \"application/json\");");
            dedent();
        }
        if (!first) {
            line("} else {");
            indent();
            line("res.status = 500;");
            line("res.set_content(error_response(\"Internal error\", 500).dump(), \"application/json\");");
            dedent();
            line("}");
        }
        dedent();
        ...
        line("server.Delete(\"/api/objects/:id\", [](const httplib::Request& req, httplib::Response& res) {");
        indent();
        line("std::string id = req.path_params.at(\"id\");");
