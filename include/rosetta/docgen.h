...
namespace rosetta {
    ...
    struct MarkdownDoc {
        ...
        template <std::meta::info Fld, auto... Anns> void field(const char *name) {
            constexpr const char *type_str = std::define_static_string(
                std::meta::display_string_of(std::meta::type_of(Fld)));
            ...
            out << "| `" << name << "` | `" << docgen_detail::prettify(type_str)
                << "` | " << desc << " |\n";
        }
    ...
    private:
        ...
        void emit_method(const char *name, bool is_static) {
            constexpr auto        dann    = ann::get_or<doc>(doc{""}, Anns...);
            constexpr const char *ret_str = std::define_static_string(
            ...
            template for (constexpr auto p :
                ...
                constexpr const char *pname =
                    std::define_static_string(std::meta::identifier_of(p));
                constexpr const char *ptype = std::define_static_string(
