namespace rosetta::em {
    ...
    // ============================================================================
    // Type Cast Registry - For converting Any back to emscripten::val
    // ============================================================================

    class TypeCastRegistry {
    public:
        using CastFunc = std::function<emscripten::val(const core::Any &)>;
        ...
        template <typename T> void register_cast() {
            ...
            if constexpr (!std::is_abstract_v<T>) {
                casts_[std::type_index(typeid(T))] = [](const core::Any &value) -> emscripten::val {
                    return emscripten::val(value.as<T>());
                };
            }
        }

        emscripten::val cast(const core::Any &value) const {
    ...
    // ============================================================================
    // Type Converter Registry - For converting emscripten::val to Any (reverse direction)
    // ============================================================================

    class TypeConverterRegistry {
    public:
        using ConverterFunc = std::function<core::Any(const emscripten::val &)>;
        ...
        template <typename T> void register_converter() {
            ...
            if constexpr (!std::is_abstract_v<T>) {
                converters_[std::type_index(typeid(T))] =
                    [](const emscripten::val &js_val) -> core::Any {
                    try {
        ...
        core::Any convert(const emscripten::val &js_val, std::type_index type) const {
    ...
    template <typename T> inline void bind_vector_type() {
        TypeCastRegistry::instance().register_custom_cast(
            std::type_index(typeid(std::vector<T>)), [](const core::Any &value) -> emscripten::val {
    ...
    template <typename Ret, typename... Args> inline void bind_function_type() {
        ...
        TypeConverterRegistry::instance().register_custom_converter(
            std::type_index(typeid(FuncType)), [](const emscripten::val &js_func) -> core::Any {
                ...
                FuncType cpp_func = [js_func](Args... args) -> Ret {
                    try {
                        ...
                        emscripten::val result = js_func(emscripten::val(args)...);
    ...
    inline emscripten::val any_to_val(const core::Any &value) {
        if (!value.has_value())
            return emscripten::val::undefined();
        ...
        if (type == std::type_index(typeid(size_t)))
            return emscripten::val(static_cast<unsigned long>(value.as<size_t>()));
        if (type == std::type_index(typeid(long)))
            return emscripten::val(value.as<long>());
        if (type == std::type_index(typeid(void)))
            return emscripten::val::undefined();
    ...
    inline core::Any val_to_any(const emscripten::val &js_val, std::type_index expected_type) {
    ...
    template <typename T> struct ClassWrapper {
        static emscripten::val getField(T &obj, const std::string &name) {
        ...
        static void setField(T &obj, const std::string &name, emscripten::val value) {
            const auto &meta = core::Registry::instance().get<T>();
            try {
        ...
        static emscripten::val callMethod(T &obj, const std::string &name, emscripten::val args) {
            const auto &meta = core::Registry::instance().get<T>();
        ...
        static emscripten::val getMetadata() {
            const auto     &meta = core::Registry::instance().get<T>();
            emscripten::val info = emscripten::val::object();

            emscripten::val fields = emscripten::val::array();
            for (const auto &f : meta.fields()) {
                fields.call<void>("push", emscripten::val(f));
            }
            info.set("fields", fields);

            emscripten::val methods = emscripten::val::array();
            for (const auto &m : meta.methods()) {
                methods.call<void>("push", emscripten::val(m));
            }
            info.set("methods", methods);

            emscripten::val bases       = emscripten::val::array();
            const auto     &inheritance = meta.inheritance();
            for (const auto &base : inheritance.base_classes) {
                bases.call<void>("push", emscripten::val(base.name));
            }
    ...
    public:
        static void bind(const std::string &js_name) {
            ...
            auto em_class = emscripten::class_<T, emscripten::base<Base>>(name.c_str());
    ...
    private:
        ...
        static void bind_all_fields(EmClass &em_class, const core::ClassMetadata<T> &meta) {
            ...
            for (const auto &field_name : meta.fields()) {
                ...
                auto getter = std::function<emscripten::val(const T &)>(
                    [field_idx](const T &obj) -> emscripten::val {
                ...
                auto setter = std::function<void(T &, emscripten::val)>(
                    [field_idx](T &obj, emscripten::val value) {
                        const std::string &fname = getFieldNameStorage()[field_idx];
                        const auto        &fmeta = core::Registry::instance().get<T>();
                        try {
            ...
            for (const auto &method_name : all_methods) {
                ...
                case 0:
                    em_class.function(method_name.c_str(),
                                      std::function<emscripten::val(T &)>(
                                          [method_idx](T &obj) -> emscripten::val {
                                              const std::string &mname =
                                                  getMethodNameStorage()[method_idx];
                                              return ClassWrapper<T>::callMethod(
                                                  obj, mname, emscripten::val::array());
                                          }));
                    break;
                ...
                case 1:
                    em_class.function(
                        method_name.c_str(),
                        std::function<emscripten::val(T &, emscripten::val)>(
                            [method_idx](T &obj, emscripten::val arg0) -> emscripten::val {
                ...
                case 2:
                    em_class.function(
                        method_name.c_str(),
                        std::function<emscripten::val(T &, emscripten::val, emscripten::val)>(
                            [method_idx](T &obj, emscripten::val arg0,
                                         emscripten::val arg1) -> emscripten::val {
                                const std::string &mname = getMethodNameStorage()[method_idx];
                                emscripten::val    args  = emscripten::val::array();
                                args.call<void>("push", arg0);
                ...
                case 3:
                    em_class.function(
                        ...
                        std::function<emscripten::val(T &, emscripten::val, emscripten::val,
                                                      emscripten::val)>(
                            [method_idx](T &obj, emscripten::val arg0, emscripten::val arg1,
                                         emscripten::val arg2) -> emscripten::val {
                                const std::string &mname = getMethodNameStorage()[method_idx];
                                emscripten::val    args  = emscripten::val::array();
                                args.call<void>("push", arg0);
                                args.call<void>("push", arg1);
                ...
                case 4:
                    em_class.function(
                        ...
                        std::function<emscripten::val(T &, emscripten::val, emscripten::val,
                            ...
                            [method_idx](T &obj, emscripten::val arg0, emscripten::val arg1,
                                         emscripten::val arg2,
                                         emscripten::val arg3) -> emscripten::val {
                                const std::string &mname = getMethodNameStorage()[method_idx];
                                emscripten::val    args  = emscripten::val::array();
                                args.call<void>("push", arg0);
                                args.call<void>("push", arg1);
                ...
                case 5:
                    em_class.function(
                        ...
                        std::function<emscripten::val(T &, emscripten::val, emscripten::val,
                                                      emscripten::val, emscripten::val,
                                                      emscripten::val)>(
                            [method_idx](T &obj, emscripten::val a0, emscripten::val a1,
                                         emscripten::val a2, emscripten::val a3,
                                         emscripten::val a4) -> emscripten::val {
                                const std::string &mname = getMethodNameStorage()[method_idx];
                                emscripten::val    args  = emscripten::val::array();
                ...
                default:
                    ...
                    em_class.function(
                        method_name.c_str(),
                        std::function<emscripten::val(T &, emscripten::val)>(
                            [method_idx](T &obj, emscripten::val args) -> emscripten::val {
                                const std::string &mname = getMethodNameStorage()[method_idx];
                                return ClassWrapper<T>::callMethod(obj, mname, args);
                            }));
                    break;
    ...
    public:
        static void bind(const std::string &js_name) {
            ...
            auto em_class = emscripten::class_<T>(name.c_str());
    ...
    // Helper to convert emscripten::val arrays to std::vector
    struct EmValHelper {
        template <typename T> static std::vector<T> toVector(const emscripten::val &arr) {
            std::vector<T> result;
        ...
        template <typename T> static emscripten::val fromVector(const std::vector<T> &vec) {
            emscripten::val arr = emscripten::val::array();
            for (size_t i = 0; i < vec.size(); ++i) {
                arr.set(i, vec[i]);
            }
    ...
    public:
        static void bind(const std::string &js_name, FactoryFunc factory) {
            ...
            em_class.constructor(factory, emscripten::allow_raw_pointers());
    ...
    inline EmGenerator &EmGenerator::bind_class(const std::string &js_name) {
        AutoBinder<T, CtorSignatures...>::bind(js_name);
        return *this;
    }

    template <typename T, typename FactoryFunc>
    inline EmGenerator &EmGenerator::bind_class_with_factory(FactoryFunc        factory,
    ...
    inline EmGenerator &EmGenerator::bind_derived_class(const std::string &js_name) {
        AutoBinderWithBase<Derived, Base, CtorSignatures...>::bind(js_name);
        return *this;
    }

    template <typename T>
    inline EmGenerator &EmGenerator::bind_abstract_class(const std::string &js_name) {
        {
            const auto &meta = core::Registry::instance().get<T>();
            std::string name = js_name.empty() ? meta.name() : js_name;
    ...
    inline EmGenerator &EmGenerator::bind_function(const std::string &name, Ret (*func)(Args...)) {
    ...
    inline EmGenerator &EmGenerator::add_utilities() {
        struct Utils {
            static emscripten::val listClasses() {
            ...
            static emscripten::val getInheritanceInfo(const std::string &class_name) {
                emscripten::val info = emscripten::val::object();

                auto bases = InheritanceRegistry::instance().get_base_classes(class_name);
                emscripten::val bases_arr = emscripten::val::array();
                ...
                emscripten::val derived_arr = emscripten::val::array();
                ...
                emscripten::val ancestors_arr = emscripten::val::array();
        ...
        emscripten::function("listClasses", &Utils::listClasses);
        emscripten::function("version", &Utils::version);
        emscripten::function("generateTypeScript", &Utils::generateTypeScript);
        emscripten::function("getInheritanceInfo", &Utils::getInheritanceInfo);
        return *this;
    }

    inline std::string EmGenerator::get_typescript_declarations() {
        return TypeScriptGenerator::instance().generate();
    }
