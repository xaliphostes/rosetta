...
private:
    ...
    static std::string substitute_variables(const std::string& input, 
...
public:
    ...
    static void generate_sample_config(const std::string& filepath) {
        ...
        // Variables section - CMake-style variable definitions
        j["variables"] = {
        ...
        j["rosetta"] = {
            {"registration_header", "${PROJECT_ROOT}/src/bindings/registration.h"},
            {"registration_namespace", "myproject_rosetta"},
            {"registration_function", "register_classes"},
            {"types_namespace", "myproject"},
            {"cpp_namespaces", {
                {"strip", true},      // Strip namespaces from binding names (default: true)
                {"separator", ""}     // Separator to replace "::" (empty = strip entirely, e.g., "_" -> "arch_Model")
            }}
        };
