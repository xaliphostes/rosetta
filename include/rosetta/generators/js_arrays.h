...
namespace rosetta {
    ...
    template <typename T, size_t N> inline void registerArrayType(JsGenerator& generator)
    {
        ...
        std::string typeName = typeid(std::array<T, N>).name();
        ...
        generator.register_type_converter(
            ...
            [](Napi::Env env, const std::any& value) -> Napi::Value {
                try {
                    const auto& arr = std::any_cast<const std::array<T, N>&>(value);
                    auto js_arr = Napi::Array::New(env, N);
                    for (size_t i = 0; i < N; ++i) {
                        js_arr.Set(i, toNapiValue<T>(env, arr[i]));
                    }
                    return js_arr;
                } catch (const std::bad_any_cast&) {
                    auto arr = std::any_cast<std::array<T, N>>(value);
                    auto js_arr = Napi::Array::New(env, N);
                    for (size_t i = 0; i < N; ++i) {
                        js_arr.Set(i, toNapiValue<T>(env, arr[i]));
                    }
            ...
            [](const Napi::Value& js_val) -> std::any {
                ...
                for (uint32_t i = 0; i < N; ++i) {
                    arr[i] = fromNapiValue<T>(js_arr.Get(i));
                }
    ...
    inline void registerArrayAlias(JsGenerator& generator)
    {
        ...
        generator.register_type_converter(
            ...
            [](Napi::Env env, const std::any& value) -> Napi::Value {
                try {
                    const auto& arr = std::any_cast<const AliasType&>(value);
                    auto js_arr = Napi::Array::New(env, N);
                    for (size_t i = 0; i < N; ++i) {
                    ...
                    auto arr = std::any_cast<AliasType>(value);
                    auto js_arr = Napi::Array::New(env, N);
                    for (size_t i = 0; i < N; ++i) {
                        js_arr.Set(i, toNapiValue<ElementType>(env, arr[i]));
                    }
