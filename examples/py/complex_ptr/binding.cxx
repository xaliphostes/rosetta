...
public:
    ...
    Surface(const std::vector<double> &positions, const std::vector<int> &indices) {
        ...
        for (size_t i = 0; i < num_points; ++i) {
            size_t idx = 3 * i;
            points.push_back(new Point{positions[idx], positions[idx + 1], positions[idx + 2]});
        }

        size_t num_triangles = indices.size() / 3;
        for (size_t i = 0; i < num_triangles; ++i) {
            size_t idx = 3 * i;
            triangles.push_back(new Triangle{indices[idx], indices[idx + 1], indices[idx + 2]});
        }
    }

    void transform(std::function<Point*(Point *)> func) {
        for (Point *p : points) {
            p = func(p);
        }
    }
...
BEGIN_PY_MODULE(complex_ptr, "Python bindings for C++ classes using Rosetta introspection") {
    ...
    rosetta::py::register_function_converter<Point*, Point*>();
